...
## Premise
The ARC-AGI-3 research log keeps "one entry per change, newest at the bottom". The page orders its entries by when each last changed on GitHub — Cards Against The Humanity (2026-09-20, oldest), ARC-AGI-3 (2026-09-24 00:04), this site at joviangame.me (2026-09-24 07:02, newest) — and grows downward: each newer entry is set larger, so the weight sits on the bottom edge and the last line is the address.
...
- Newest entry size x = min(content width ÷ 7.6, 160px). Each older entry is divided by r (r = 2 at ≥ 720px, 1.5 below): 160 / 80 / 40px at desktop; the oldest never below 16px.
...
## Colour
- Canvas `#0d1521`.
- Tertiary — oldest entry (Cards Against The Humanity) `#8195ae` — ≈ 15% of ink — 6.0 : 1.
- Secondary — middle entry (ARC-AGI-3) `#d8c6a2` — ≈ 30% — 10.9 : 1.
- Primary — newest entry (JustAdev742, joviangame.me) `#ffb347` — ≈ 55% — 10.3 : 1.
...
- At desktop the oldest entry is set at 40px.
